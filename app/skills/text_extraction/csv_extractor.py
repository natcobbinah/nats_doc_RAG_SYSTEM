...
class TextFromCsv(IDocumentHandler):
    def __init__(self):
    ...
    def extract_content_from_document(self, stream: io.BytesIO, blob_name: str, source_url: str) -> List[Dict]:
        ...
        try:
            stream.seek(0)
            text_data = stream.read().decode("utf-8", errors="ignore")
            ...
            log(
                type=LogLevel.ERROR,
                message={
                    "message": f"Failed to extract content from CSV file, {blob_name}",
...
if __name__ == "__main__":
    dummy_file_path = os.path.join(os.path.dirname(__file__), "examples/job_title_des.csv")
